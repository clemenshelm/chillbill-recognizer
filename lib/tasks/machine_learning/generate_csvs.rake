namespace :machine_learning do
  ...
  task :generate_csvs do
    ...
    CSV.open('data/prices.csv', 'wb') do |prices_csv|
      CSV.open('data/correct_price_tuples.csv', 'wb') do |correct_price_tuples_csv|
        ...
        bills.each do |bill|
          ...
          total_prices = existing_total_prices.map { |key, price| price }.each(&add_id)
          vat_prices = bill['vat_prices'].map { |key, price| price }.each(&add_id)
          remaining_prices = bill['remaining_prices'].each(&method(:add_id))
          all_prices = total_prices + vat_prices + bill['remaining_prices']
