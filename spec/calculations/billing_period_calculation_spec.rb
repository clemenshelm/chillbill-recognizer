...
describe BillingPeriodCalculation do
  ...
  it 'returns the billing period attributes in the correct format' do
    ...
    calculated_billing_period = BillingPeriodCalculation.new(billing_period_terms).billing_period
    expect(calculated_billing_period[:from]).to eq DateTime.iso8601('2015-03-01')
    expect(calculated_billing_period[:to]).to eq DateTime.iso8601('2015-03-31')
  end
